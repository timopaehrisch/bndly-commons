...
public class StreamingObjectIdMappingPostInterceptor implements MappingPostInterceptor {

	@Override
	public void postIntercept(MappingState state) {
		Object source = state.getSource();
		if (StreamingObject.class.isInstance(source)) {
			StreamingObject ar = (StreamingObject) source;
			new SetterBeanPropertyWriter().set("id", ar.getId(), state.getTarget());
		}
	}
