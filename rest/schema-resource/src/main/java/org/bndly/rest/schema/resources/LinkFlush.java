...
public class LinkFlush implements AtomLinkInjectorListener {
	@Reference
	...
	private void doFlushIfRequired(AtomLinkDescription atomLinkDescription) {
		...
		if (
				SchemaList.class.equals(atomLinkDescription.getLinkedInClass())
				|| SchemaBean.class.equals(atomLinkDescription.getLinkedInClass())
				|| TypeBean.class.equals(atomLinkDescription.getLinkedInClass())
		) {
			try (CacheTransaction tx = factory.createCacheTransaction()) {
				tx.flushRecursive("/schema");
			}
		}
	}
}
