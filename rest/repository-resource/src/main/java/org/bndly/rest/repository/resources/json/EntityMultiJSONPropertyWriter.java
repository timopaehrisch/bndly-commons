 ...
 */
public class EntityMultiJSONPropertyWriter implements JSONPropertyWriter {

	@Override
	public void write(Property property, JSONWriter writer) throws RepositoryException, IOException {
		Record[] entities = property.getEntities();
		if (entities != null) {
			...
			for (Record entity : entities) {
				...
				if (entity != null) {
					writer.writeString(entity.getType().getName() + ";" + entity.getId());
				} else {
					writer.writeNull();
				}
				firstString = false;
