 ...
 */
public class PersistedEventForRecordContext implements TransactionAppender {

	public static final PersistedEventForRecordContext INSTANCE = new PersistedEventForRecordContext();
	...
	public void append(Transaction tx, final Record record, EngineImpl engine) {
		tx.afterCommit(new CommitHandler() {
			...
			public void didCommit(Transaction transaction) {
				record.getContext().persisted(record);
			}
		});
	}
