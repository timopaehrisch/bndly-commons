...
public class TableExpressionImpl extends ContextRelatedQueryComponentImpl implements TableExpression {
	...
	public TableExpression table(String tableName, String alias) {
		getQueryContext().useTable(tableName, alias);
	...
	public Join join(String tableName, String alias) {
		getQueryContext().useTable(tableName, alias);
		Join j = new JoinImpl(getQueryContext(), getVendorConfiguration());
